...
pd.options.display.float_format = '{:.2f}'.format #to show float in decimal instead of scientific
#read data
...
df.groupby('partnum').aggregate({'revenue':np.sum}).sort_values(by='revenue', ascending=False)
